 ...
 * 管理员 不通过课程审核
 */
public class AdminPassTestCase extends BaseTestCase {

    @BeforeClass
    public void beforeClass() {
        superAdmin.getManage().lessonAgent().lessonReply(lessonId, Global_enum.PUB_TYPE.PASS);
    }

    @Test(description = "教师后台 课程列表")
    public void teacher_lesson_list_test() {
        teacher_list(lessonId).ifPresent(i -> {
            ass_tls(STATE_LESSON_APPLY_PASS, i.getString("state"));
        });
    }
    ...
    public void teacher_lessonInfo_test() {
        ...
        info.array.flatMap(array -> JsonFuncUtil.extract(array, classroomId, "classroomId")).ifPresent(i -> {
            ass_tcp(PUB_APPLY_PASS, i.getString("pubType"));
        });
    }
    ...
    public void superAdmin_lesson_list_test() {
        superAdmin_list(lessonId).ifPresent(i -> {
            ass_sls(STATE_LESSON_APPLY_PASS, i.getString("state"));
        });
    }
    ...
    public void superAdmin_lessonInfo_test() {
        ...
        info.array.flatMap(array -> JsonFuncUtil.extract(array, classroomId, "classroomId")).ifPresent(i -> {
            ass_scp(PUB_APPLY_PASS, i.getString("pubType"));
        });
    }
